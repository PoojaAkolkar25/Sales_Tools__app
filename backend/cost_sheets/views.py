...
class CostSheetViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            logger.error(f"Error in perform_create (CostSheetViewSet): {str(e)}", exc_info=True)
            raise
    ...
    def approve(self, request, pk=None):
        try:
            ...
            logger.error(f"Error in approve (CostSheetViewSet): {str(e)}", exc_info=True)
    ...
    def update(self, request, *args, **kwargs):
        try:
            ...
            logger.error(f"Error in update (CostSheetViewSet): {str(e)}", exc_info=True)
            return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def upload_attachment(self, request, pk=None):
        try:
            ...
            logger.error(f"Error in upload_attachment (CostSheetViewSet): {str(e)}", exc_info=True)
            return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def export_pdf(self, request):
        try:
            ...
            # Calculate margin pct for the template context
            cost_sheets = []
            for cs in queryset:
                margin_pct = 0
                ...
                cs.margin_pct = margin_pct
                cost_sheets.append(cs)

            html_string = render_to_string('cost_sheets/report_pdf.html', {'cost_sheets': cost_sheets, 'now': timezone.now()})
            
            result = io.BytesIO()
            pisa_status = pisa.CreatePDF(html_string, dest=result)
            
            if pisa_status.err:
                logger.error("PDF generation error occurred in export_pdf")
                return Response({
                    "status": "error",
            ...
            response['Content-Disposition'] = f'attachment; filename="cost_sheets_report_{today}.pdf"'
            return response
        except Exception as e:
            logger.error(f"Error in export_pdf (CostSheetViewSet): {str(e)}", exc_info=True)
            return Response({
                "status": "error",
    ...
    def export_excel(self, request):
        try:
            ...
            logger.error(f"Error in export_excel (CostSheetViewSet): {str(e)}", exc_info=True)
            return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
